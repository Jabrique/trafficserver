...
- **AVIF (Q=90):** 123 KB (High quality, larger than original)
...
```
# Global activation with custom defaults
webp_transform.so convert_to_avif convert_to_webp avif_quality=60 webp_quality=80
```

### 2. Per-Remap Configuration
Add the plugin to specific rules in `remap.config` to have different settings for different sites or paths:
...
map http://img.com/thumb/ http://origin-img/ @plugin=webp_transform.so @pparam=convert_to_avif @pparam=avif_quality=15
```
...
2.  **Selection:**
    ...
    - If neither supported -> Targets **JPEG** (if input was modern) or passes through original.
3.  **Transformation:** ImageMagick performs the actual conversion based on the instance-specific quality settings.
